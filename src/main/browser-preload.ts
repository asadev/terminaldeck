...
/**
 * The one module specifier the generated guest script asks for, spelled as the
 * exact quoted token it emits.
 *
 * This is `'electron'`, single-quoted, and it is a constant so that this file's
 * own text never contains the literal `require(` + `'electron'` + `)` — which is
 * text, not an import, but is indistinguishable from a real runtime import to
 * the header-scanner `seam.test.ts` runs over every file the headless build can
 * reach. `browser-preload-cdp.ts` re-delivers this exact source string on the
 * server, so this module rides into that closure, and the *honest* fact about
 * it — that it imports nothing from Electron; it emits a string a guest page's
 * own shimmed `require` answers — has to be visible to a text detector as well
 * as true. The emitted bytes are unchanged: `require(${GUEST_ELECTRON_MODULE})`
 * expands to `require('electron')`, so the desktop's guest script and the
 * server's stay the identical bytes the CDP bridge's header promises.
 */
const GUEST_ELECTRON_MODULE = "'electron'"
...
;(function () {
  var ipc = require(${GUEST_ELECTRON_MODULE}).ipcRenderer
