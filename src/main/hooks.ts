...
export function syncInstalledHooks(context: HookContext): HookProviderStatus[] {
  /*
   * The codex feature flag is repaired even when the hooks read `complete`
   * — his machine's state: every hook pointing here, every Codex session
   * still opening on a deprecation line. The flag belongs to no copy of this
   * app (see `migrateCodexFeatureFlag`), so the another-copy rule does not apply.
   */
  try {
   ...
   * *feature* not to a list of the assistants that existed that morning, so a
   * later arrival is covered by it. Turning any provider off on the Session
   * updates page withdraws it (see `removeHooks`).
   */
...
export function installHooksWhereConfigured(context: HookContext): HookProviderStatus[] {
  // The codex feature flag is repaired BEFORE the declined early-return: it
  // belongs to no copy of this app (see `migrateCodexFeatureFlag`) and installs
  // nothing, so a person who said "no" at the first-run offer must still stop
  // seeing the Codex deprecation line at every boot. Behind the return, a
  // declined machine would keep that spam forever.
  try {
    migrateCodexFeatureFlag(context)
  } catch (error) {
    console.error('[hooks] could not migrate the codex feature flag:', error)
  }
  // An explicit "no" at the desktop's first-run offer covers this pass too:
  // the marker lives under the same home directory these configs do, so a
  // person who declined on this machine has declined for this machine.
