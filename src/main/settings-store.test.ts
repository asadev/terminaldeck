...
/**
 * The store half of the settings, exercised with **no Electron anywhere**.
 *
 * That is the whole point of the split: these functions took `app.getPath` and
 * now take the user-data directory `platform/paths.ts` answers, so this file
 * installs `nodePaths` — the plain-Node paths the headless daemon installs — and
 * never mocks `electron`. If a runtime Electron import creeps back into this
 * half, this file cannot even import.
 *
 * The cases themselves are the same ones that used to live in
 * `settings-extra.test.ts`: the settings file is the only copy of choices a user
 * made by hand, so these are about the ways it can be lost — a corrupt file
 * overwritten with an empty one, a newer build's keys dropped by an older one, a
 * write interrupted halfway. The happy path is one line.
 */

const ROOT = mkdtempSync(join(tmpdir(), 'td-settings-store-'))
 ...
 *
 * The one snapshot case that reads `configPaths()` — the Advanced list — lives
 * in `settings-extra.test.ts`, because that list is the shell half.
 */
