...
describe('effort and fast mode from the settings the CLI itself writes', () => {
  ...
  it('follows the documented fastMode rule: absent or false means off', () => {
    expect(fastFromSettings({}).value).toBe('off')
    expect(fastFromSettings({ fastMode: false }).value).toBe('off')
