...
function registerIpc(): void {
   ...
   * this process owns, while `machines.sendFile` has to answer for a link that
   * may be reconnecting.
   */
  installDownloads({
    userData: () => app.getPath('userData'),
    ...
    deliver: async (machineId, localPath, folder) => {
      if (serverStore.get(machineId) !== null) {
        try {
          return { ok: true, path: await serverConnections.sendFile(machineId, localPath, folder) }
        } catch (error) {
          return {
            ok: false,
      ...
      return await machines.sendFile(machineId, localPath, folder === '' ? undefined : folder)
    },
  })
