...
var wrapCipher = /* @__NO_SIDE_EFFECTS__ */ (params2, constructor) => {
  function wrappedCipher(key, ...args) {
    ...
    if (params2.nonceLength !== void 0) {
      const nonce = args[0];
      abytes(nonce, params2.varSizeNonce ? void 0 : params2.nonceLength, "nonce");
    }
    const tagl = params2.tagLength;
    const aadStart = params2.nonceLength !== void 0 ? 1 : 0;
    if (!params2.withAAD) {
      for (let i = aadStart; i < args.length; i++)
        if (isBytes(args[i]))
          throw new Error("AAD not supported");
    }
    if (params2.withAAD && args[aadStart] !== void 0)
...
function runCipher(core, sigma, key, nonce, data, output2, counter, rounds) {
  ...
  const b32 = u32(block);
  const isAligned = isLE && isAligned32(data) && isAligned32(output2);
...
function createCipher(core, opts) {
  ...
  return (key, nonce, data, output2, counter = 0) => {
    ...
    abytes(data, void 0, "data");
    const len = data.length;
    output2 = getOutput(len, output2, false);
    anumber(counter);
...
// src/shared/sealed.ts
var NOISE_NAME = "Noise_IK_25519_ChaChaPoly_SHA256";
var SEALED_VERSION = 1;
var KEY_BYTES = 32;
var NONCE_BYTES = 12;
var TAG_BYTES = 16;
var HASH_BYTES = 32;
var MAX_COUNTER2 = 2 ** 48;
var SealedRefusal = class extends Error {
  constructor(message) {
    super(message);
...
var SealedTransport = class {
  ...
  receiveCounter = 0;
  send(plaintext) {
    if (this.sendCounter >= MAX_COUNTER2) throw new SealedRefusal("sealed channel exhausted");
    const frame = seal(this.sendKey, this.sendCounter, plaintext, Buffer.alloc(0));
    this.sendCounter += 1;
    return frame;
  }
  receive(sealed) {
    if (this.receiveCounter >= MAX_COUNTER2) throw new SealedRefusal("sealed channel exhausted");
    let plaintext;
    try {
...
function finishHandshake(pending, reply) {
...
var RELAY_GUEST_PATH = "/v1/join";
var CHANNEL_BYTES = 16;
var ENVELOPE_HEADER = 1 + CHANNEL_BYTES;
var MAX_PAYLOAD_BYTES = 96 * 1024;
var RELAY_SEALED_VERSION = SEALED_VERSION;
var NOISE_MESSAGE_BYTES = 80;
var NOISE_REPLY_BYTES = 48;
var HANDSHAKE_OPEN_BYTES = 1 + NOISE_MESSAGE_BYTES;
var HANDSHAKE_REPLY_BYTES = 1 + NOISE_REPLY_BYTES;
...
var CAPABILITIES = [CAPABILITY.localhost, CAPABILITY.create, CAPABILITY.upload];
var MAX_NET_CHUNK_BYTES = 24 * 1024;
var MAX_NET_DATA_CHARS = Math.ceil(MAX_NET_CHUNK_BYTES / 3) * 4;
var NET_WINDOW_BYTES = 256 * 1024;
var MAX_UPLOAD_BYTES = 512 * 1024 * 1024;
var MAX_MESSAGE_BYTES = 64 * 1024;
var MAX_INPUT_BYTES = 16 * 1024;
var OUTPUT_CHUNK_BYTES = 32 * 1024;

// .harness/guest/guest.ts
var uri = process.argv[2];
if (!uri) throw new Error("pass the pairing URI");
var params = new URLSearchParams(uri.slice(uri.indexOf("?") + 1));
var relay = params.get("r");
var hostId = params.get("h");
var hostKey = Buffer.from(params.get("k"), "base64url");
var token = params.get("t");
var url = `${relay}${RELAY_GUEST_PATH}?host=${encodeURIComponent(hostId)}`;
console.log(`[guest] dialling ${url}`);
var KEYFILE = new URL("./device-key.json", import.meta.url).pathname;
var CREDFILE = new URL("./credential.txt", import.meta.url).pathname;
var device;
...
var started = startHandshake(device, hostKey);
